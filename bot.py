...
async def bump_all_ads(callback: CallbackQuery):
    ...
    for row in ads_rows:
        ...
        if not is_admin(user_id):
            remaining_time = get_cooldown_remaining(ad["last_bump"])
            if remaining_time:
                skipped_details.append(f"• ID {ad_id}: через {remaining_time}")
                continue
        ...
        try:
            ...
            bumped_count += 1
            await asyncio.sleep(1)  # Защита от спам-лимитов
        except Exception as e:
    ...
    report_text = ""
    if bumped_count > 0:
        report_text += f"✅ Успешно поднято: {bumped_count} объявление(й).\n"
    
    if skipped_details:
        report_text += "\n⏳ Пропущено по КД:\n" + "\n".join(skipped_details)
    else:
        if bumped_count == 0:
            report_text = "⏳ Ни одного объявления не поднято. Все еще действует КД."
...
async def main():
    ...
    # Сброс старых подключений (принудительно закрываем конфликтующие сессии)
