...
async def repost_ad(ad, new_price=None, old_price=None):
    ...
    return sent[0].message_id, button_msg.message_id
...
async def bump(callback: CallbackQuery):
    ...
    new_msg_id, new_button_id = await repost_ad(ad)
...
async def bump_all(callback: CallbackQuery):
    ...
    for row in rows:
        ...
        if not check_cooldown(ad["last_bump"]):
            continue
...
async def save_new_price(message: Message, state: FSMContext):
    ...
    new_msg_id, new_button_id = await repost_ad(
        ad,
