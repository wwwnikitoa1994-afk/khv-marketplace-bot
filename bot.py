...
# Блокировка для безопасной работы SQLite на сервере Render
...
async def publish_post(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Ошибка публикации объявления. Попробуйте позже.")
        print(f"Publish error: {e}")

# =========================================
# GENERATE KEYBOARD FOR MY ADS
# =========================================
...
def my_ads_keyboard(ads, user_id):
    ...
    if ads:
        ...
        if any_available:
            ...
            keyboard.append([InlineKeyboardButton(text="🔒 Поднять все (Недоступно)", callback_data="bump_all_locked")])
...
async def open_ad(callback: CallbackQuery):
    ...
    if cooldown_time:
        bump_text = f"🔒 Поднять (через {cooldown_time})"
        edit_text = f"🔒 Изменить цену (через {cooldown_time})"
    else:
        bump_text = "🔄 Поднять"
...
async def edit_price(callback: CallbackQuery, state: FSMContext):
    ...
    if cooldown_time:
        await callback.answer(f"🔒 Изменить цену нельзя! Можно будет изменить через: {cooldown_time}", show_alert=True)
        return
...
# =========================================
# BUMP SINGLE AD (CORRECTED MIXED ALBUMS)
# =========================================
...
async def bump_ad(callback: CallbackQuery):
    ...
    if cooldown_time:
        await callback.answer(f"🔒 Поднять нельзя! Можно будет поднять через: {cooldown_time}", show_alert=True)
        return
...
# =========================================
# BUMP ALL HANDLING (FULLY WORKING)
# =========================================
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    for row in ad_rows:
        ...
        try:
            ...
            await asyncio.sleep(1) # Защита от Flood Control Телеграма
        except Exception as e:
            print(f"Bump all error for ad {ad['id']}: {e}")

    report_text = f"📊 <b>Результат массового поднятия:</b>\n\n"
    report_text += f"✅ Успешно поднято: <b>{bumped_count}</b>\n"
    if bumped_titles:
        report_text += "\n".join(bumped_titles) + "\n"
