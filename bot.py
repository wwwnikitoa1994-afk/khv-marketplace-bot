...
# =========================================================
# НОВАЯ СБОРКА КЛАВИАТУРЫ СПИСКА (РАБОТАЕТ НА 100%)
# =========================================================

def get_my_ads_keyboard(ads, user_id):
    """Сгенерировано вручную. Рассчитывает КД до ближайшего объявления."""
    bump_all_text = "🔄 Поднять все доступные"
    
    if not is_admin(user_id) and ads:
        cooldown_seconds_list = []
        has_available = False

        for ad in ads:
            # ad[2] — это строго last_bump из выборки (id, title, last_bump)
            try:
                last_bump_dt = datetime.fromisoformat(ad[2])
                rem_td = (last_bump_dt + timedelta(hours=COOLDOWN_HOURS)) - datetime.now()
                total_sec = rem_td.total_seconds()
                
                if total_sec > 0:
                    cooldown_seconds_list.append(total_sec)
                else:
                    has_available = True
            except Exception:
                has_available = True

        # Выбираем текст кнопки на основе наличия доступных постов
        if has_available:
            # Если хотя бы одно объявление вышло из КД — кнопка пишется как доступная
            bump_all_text = "🔄 Поднять все доступные"
        else:
            # Если вообще все объявления на КД — ищем самое близкое к выходу
            if cooldown_seconds_list:
                min_sec = min(cooldown_seconds_list)
                hours, remainder = divmod(int(min_sec), 3600)
                minutes, _ = divmod(remainder, 60)
                bump_all_text = f"⏳ Поднять все (через {hours}ч {minutes}м)"
...
def get_single_ad_keyboard(ad_id, last_bump, user_id):
    """Генерирует инлайн-кнопки управления одним постом с КД на тексте"""
...
async def edit_price(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(callback.from_user.id):
        ...
        if remaining_time:
            # Текст алерта на случай обхода интерфейса
...
# =========================================================
# НОВЫЙ ЧИСТЫЙ ХЕНДЛЕР МАССОВОГО ПОДНЯТИЯ (СОБРАН ВРУЧНУЮ)
# =========================================================
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    # Распределяем по спискам на основе строгого математического КД
    for row in ads_rows:
        ...
        if is_admin(user_id):
            ...
            try:
                last_bump_dt = datetime.fromisoformat(ad["last_bump"])
                rem_td = (last_bump_dt + timedelta(hours=COOLDOWN_HOURS)) - datetime.now()
                total_sec = rem_td.total_seconds()
                
                if total_sec > 0:
                    cooldown_seconds_list.append(total_sec)
                    # Форматируем красивую строку для отчета
                    hours, remainder = divmod(int(total_sec), 3600)
                    minutes, _ = divmod(remainder, 60)
                    skipped_details.append(f"• ID {ad['id']}: еще {hours}ч {minutes}м")
                else:
                    available_ads.append(ad)
            except Exception:
                available_ads.append(ad)

    # КЕЙС 1: Если доступных объявлений вообще нет (все на КД)
    if not available_ads:
        min_sec = min(cooldown_seconds_list) if cooldown_seconds_list else 0
        hours, remainder = divmod(int(min_sec), 3600)
        minutes, _ = divmod(remainder, 60)
        
        await callback.answer(
            f"⏳ Ни одно объявление не поднято.\nБлижайшее будет доступно через: {hours}ч {minutes}м", 
            show_alert=True
        )
        return

    # КЕЙС 2: Есть доступные посты — поднимаем их цепочкой
    bumped_count = 0
    for ad in available_ads:
        ...
        try:
            ...
            bumped_count += 1
            await asyncio.sleep(1)  # Защита от Flood Control Telegram
        except Exception as e:
            print(f"Error bumping ad {ad_id}: {e}")

    # Обновляем меню списка
    async with db_lock:
        cursor.execute("SELECT id, title, last_bump FROM ads WHERE user_id = ? ORDER BY id DESC", (user_id,))
        updated_ads = cursor.fetchall()

    await callback.message.edit_text("📂 Ваши объявления:", reply_markup=get_my_ads_keyboard(updated_ads, user_id))

    # Вывод финального отчета во всплывающем окне
    report_text = f"✅ Успешно поднято объявлений: {bumped_count}.\n"
    if skipped_details:
        report_text += "\n⏳ Остались на КД:\n" + "\n".join(skipped_details)
    
    await callback.answer(report_text, show_alert=True)
