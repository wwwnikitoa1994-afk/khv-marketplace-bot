...
# Блокировка базы для стабильности SQLite на Render
...
# Список колонок для удобной конвертации строк БД в словари
DB_COLUMNS = [
...
def build_hashtags(action, category):
    hashtags = ""
    if "Продам" in action: hashtags += "#prodam "
    elif "Куплю" in action: hashtags += "#kuplyu "
    elif "Отдам" in action: hashtags += "#otdam "
    elif "Обменяю" in action: hashtags += "#obmen "
...
async def publish_post(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Ошибка публикации объявления.")
...
# =========================================
# MANAGEMENT: BUMP (FIXED FOR ALBUMS)
# =========================================
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    for row in ad_rows:
        ...
        try:
            ...
            await asyncio.sleep(1.5)  # Защита от лимитов (Flood control) Telegram
        except Exception as e:
...
async def main():
    ...
    # Принудительно сбрасываем старые соединения, спасая от TelegramConflictError
