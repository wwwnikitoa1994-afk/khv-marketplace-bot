...
COOLDOWN_HOURS = 24  # Кулдаун изменен на 24 часа
...
def build_caption(ad):
    # ТУТ ИСПРАВЛЕНА СИНТАКСИЧЕСКАЯ ОШИБКА С ОПЕЧАТКОЙ В СКОБКЕ
...
def my_ads_keyboard(ads, user_id):
    ...
    if ads:
        any_available = False
        for ad_row in ads:
            if not check_cooldown(ad_row[2]) and not is_admin(user_id):
                pass
            else:
                any_available = True
        if any_available:
...
async def bump_ad(callback: CallbackQuery):
    ...
    if cooldown_time:
        await callback.answer(f"🔒 Поднять нельзя! Оставшееся время: {cooldown_time}", show_alert=True)
        return
