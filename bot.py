...
TOKEN = "8634367728:AAHgUDNd75cLZMwmh2VR0xj0Rl7H3DxwEyQ"
...
# =========================================
# DATABASE & PERSISTENT FSM STORAGE
# =========================================
...
# Таблица состояний FSM (чтобы карточки не терялись при перезапуске)
cursor.execute("""
...
class SQLiteStorage(BaseStorage):
    def __init__(self, db_connection, lock):
    ...
    async def set_state(self, key: StorageKey, state: StateType = None) -> None:
        ...
        async with self.lock:
            ...
            cursor.execute("SELECT 1 FROM fsm_states WHERE user_id = ?", (key.user_id,))
            exists = cursor.fetchone()
            
            if exists:
                cursor.execute("UPDATE fsm_states SET state = ? WHERE user_id = ?", (state_str, key.user_id))
            else:
                cursor.execute("INSERT INTO fsm_states (user_id, state, data) VALUES (?, ?, ?)", (key.user_id, state_str, '{}'))
            self.conn.commit()

    async def get_state(self, key: StorageKey) -> str | None:
        async with self.lock:
            cursor = self.conn.cursor()
            cursor.execute("SELECT state FROM fsm_states WHERE user_id = ?", (key.user_id,))
            row = cursor.fetchone()
    ...
    async def set_data(self, key: StorageKey, data: dict) -> None:
        ...
        async with self.lock:
            ...
            if exists:
                cursor.execute("UPDATE fsm_states SET data = ? WHERE user_id = ?", (data_str, key.user_id))
            else:
                cursor.execute("INSERT INTO fsm_states (user_id, state, data) VALUES (?, ?, ?)", (key.user_id, None, data_str))
            self.conn.commit()

    async def get_data(self, key: StorageKey) -> dict:
        async with self.lock:
            cursor = self.conn.cursor()
            cursor.execute("SELECT data FROM fsm_states WHERE user_id = ?", (key.user_id,))
            row = cursor.fetchone()
    ...
    async def clear(self, key: StorageKey) -> None:
        async with self.lock:
            cursor = self.conn.cursor()
            # Мягкий сброс: зануляем состояние и очищаем JSON, чтобы не ломать индексы при повторной публикации
            cursor.execute("UPDATE fsm_states SET state = NULL, data = '{}' WHERE user_id = ?", (key.user_id,))
            self.conn.commit()
...
storage = SQLiteStorage(conn, db_lock)
...
def build_caption(ad):
    ...
    # 1. Заголовок (Смайлы строго снаружи тегов)
    caption = f"{action_emoji}<b>{action_text.upper()}</b> • {category_emoji}<i>{category_text}</i>\n\n"
    
    # 2. Название товара (ЖИРНЫЙ + ПОДЧЕРКНУТЫЙ)
    caption += f"📌 <b><u>{ad.get('title', '')}</u></b>\n\n"
    
    # 3. Характеристики
    caption += f"📝 {ad.get('description', '')}\n"
    
    if ad.get("condition") and str(ad['condition']).strip():
        caption += f"📦 <i>Состояние:</i> <b>{ad['condition']}</b>\n"

    if ad.get("price") and str(ad['price']).strip():
        if ad.get("old_price") and "ПРОДАМ" in action_text.upper():
    ...
    if ad.get("exchange") and str(ad['exchange']).strip():
...
@dp.message(F.text == "➕ Опубликовать объявительный")
...
async def get_price(message: Message, state: FSMContext):
    # Явно инициализируем пустой массив фотографий, если пользователь дошел до этого шага
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    for row in ad_rows:
        ...
        if remaining_time:
            ...
            try:
                ...
                await bot.send_message(chat_id=CHANNEL_ID, text="🛒 KHV Marketplace", reply_markup=button, disable_web_page_preview=True)

                async with db_lock:
                    cursor.execute("UPDATE ads SET message_id = ?, last_bump = ? WHERE id = ?", (new_message_id, datetime.now().isoformat(), ad["id"]))
                    conn.commit()
                bumped_count += 1
            except Exception as e:
    ...
    if bumped_count > 0:
        ...
        alert_text += "⏳ Ни одно объявление не поднято.\n\n"
    ...
    async with db_lock:
        cursor.execute("SELECT id, title, last_bump FROM ads WHERE user_id = ? ORDER BY id DESC", (user_id,))
        fresh_ads = cursor.fetchall()

    try:
        await callback.message.edit_text("📂 Ваши объявления:", reply_markup=get_my_ads_keyboard(fresh_ads, user_id))
    except Exception:
        pass
...
async def save_new_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        async with db_lock:
            cursor.execute("UPDATE ads SET price = ?, old_price = ?, message_id = ?, last_bump = ? WHERE id = ?", (new_price, save_old_price, new_message_id, datetime.now().isoformat(), ad_id))
            conn.commit()
...
async def bump_ad(callback: CallbackQuery):
    ...
    try:
        ...
        async with db_lock:
            cursor.execute("UPDATE ads SET message_id = ?, last_bump = ? WHERE id = ?", (new_message_id, datetime.now().isoformat(), ad_id))
            conn.commit()
