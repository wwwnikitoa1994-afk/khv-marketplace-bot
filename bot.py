...
def get_cooldown_remaining(last_bump, user_id):
    """Возвращает строку с оставшимся временем КД. Для админов КД всегда 0."""
    if is_admin(user_id):
        return None
    try:
...
async def get_description(message: Message, state: FSMContext):
    ...
    if action in ["🟢 Продам", "🟠 Обменяю"]:
        ...
        await message.answer("📷 Отправьте от 1 до 15 photo", reply_markup=photo_keyboard)
...
def my_ads_keyboard(ads, user_id):
    ...
    # Кнопка "Поднять все" выводится только если у пользователя РЕАЛЬНО есть объявления
    if ads:
...
async def my_ads(message: Message):
    ...
    if not ads:
        await message.answer("📭 У вас нет активных объявлений. Кнопка закрытия и поднятия неактивна.")
        return
...
async def confirm_close_ad(callback: CallbackQuery):
    ...
    if not ad:
        await callback.answer("❌ Объявление не найдено или уже неактивно", show_alert=True)
        return
    ...
    if not ads:
        await callback.message.edit_text("📭 У вас больше нет активных объявлений.")
        return
    await callback.message.edit_text("📂 Ваши объявления:", reply_markup=my_ads_keyboard(ads, callback.from_user.id))
...
async def edit_price(callback: CallbackQuery, state: FSMContext):
    ...
    if cooldown_time:
        await callback.answer(f"⏳ Изменить цену нельзя! Оставшееся время КД: {cooldown_time}", show_alert=True)
        return
...
async def bump_ad(callback: CallbackQuery):
    ...
    if cooldown_time:
        await callback.answer(f"⏳ Поднять нельзя! Оставшееся время КД: {cooldown_time}", show_alert=True)
        return
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    if not ad_rows:
        await callback.answer("📭 У вас нет активных объявлений. Поднять все неактивно.", show_alert=True)
        return
    ...
    for row in ad_rows:
        ...
        if cooldown_time:
            skipped_times.append(cooldown_time)
            continue
        ...
        try:
            ...
            bumped_count += 1
            await asyncio.sleep(1) # Небольшая задержка, чтобы не спамить API Telegram
        except Exception as e:
            print(f"Bump all error for ad {ad['id']}: {e}")

    if bumped_count > 0:
        msg = f"✅ Успешно поднято объявлений: {bumped_count}."
        if skipped_times:
            msg += f"\n⚠️ Пропущено объявлений на КД: {len(skipped_times)}."
        await callback.answer(msg, show_alert=True)
    else:
        # Если ничего не подняли, выведем минимальное оставшееся КД
        await callback.answer(f"⏳ Все объявления на КД! Ближайшее КД снимется через: {min(skipped_times)}", show_alert=True)
