...
async def bump_all_ads(callback: CallbackQuery):
    ...
    # 1. Проверяем, есть ли хоть одно доступное объявление
    for row in ads_rows:
    ...
    # 2. Если НЕТ доступных вообще, выводим красивое КД до ближайшего объявления
    ...
    # 3. Если ЕСТЬ доступные, поднимаем только их
    for row in ads_rows:
        ad = dict(zip(DB_COLUMNS, row))
        ad_id = ad["id"]
    ...
    # Пересобираем меню «Мои объявления»
    async with db_lock:
