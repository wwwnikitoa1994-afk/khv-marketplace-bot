...
COOLDOWN_HOURS = 24  # Изменено на 24 часа по твоему запросу
...
def build_caption(ad):
    ...
    hashtags = ad.get("hashtags") if ad.get("hashtags") else build_hashtags(ad.get("action", ""), ad.get("category", ""))
...
# =========================================
# MANAGEMENT: BUMP ALL (WITH REFRESH TIMER)
# =========================================
...
async def bump_all_locked(callback: CallbackQuery):
    ...
    if times:
        await callback.answer(f"🔒 Все объявления на КД! Ближайшее освободится через: {min(times)}", show_alert=True)
    else:
        await callback.answer("⏳ Нет доступных объявлений для поднятия.", show_alert=True)

@dp.callback_query(F.data == "bump_all")
async def bump_all_ads(callback: CallbackQuery):
    async with db_lock:
        cursor.execute("""
        SELECT id, user_id, username, action, category, title, description, condition, price, old_price, exchange, photos, message_id, last_bump, status 
        FROM ads 
    ...
    for row in ad_rows:
        ...
        last_bump = ad["last_bump"]

        # Проверка кулдауна (24 часа)
        cooldown_time = get_cooldown_remaining(last_bump, callback.from_user.id)
        if cooldown_time:
            skipped_list.append(f"• <b>{ad['title']}</b> (доступно через: ⏳ {cooldown_time})")
            continue
        ...
        try:
            ...
            bumped_count += 1
            bumped_titles.append(f"• ✅ <b>{ad['title']}</b> — Успешно поднято!")
            await asyncio.sleep(1.5)  # Защита от Flood limit лимитов Telegram
        except Exception as e:
            print(f"Ошибка массового поднятия товара {ad['id']}: {e}")

    # Формирование информативного финального отчета
    report_text = "📊 <b>Результаты массового поднятия:</b>\n\n"
    
    if bumped_titles:
        report_text += "<b>Обновленные объявления:</b>\n" + "\n".join(bumped_titles) + "\n\n"
        
    if skipped_list:
        report_text += "<b>Не обновлены (активен таймер):</b>\n" + "\n".join(skipped_list)
    else:
        if bumped_count > 0:
            report_text += "✨ Отлично! Все ваши объявления были успешно обновлены в канале."

    keyboard = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="⬅️ Назад к списку", callback_data="back_ads")
    ]])
    
    await callback.message.edit_text(text=report_text, reply_markup=keyboard)
    await callback.answer("Массовое обновление завершено!")
