...
def get_cooldown_remaining(last_bump):
    """Возвращает оставшееся время ограничения строкой или None, если время вышло"""
    try:
...
# =========================================================
# ОБНОВЛЕННАЯ КЛАВИАТУРА СПИСКА (КНОПКА МАССОВОГО ПОДНЯТИЯ)
# =========================================================

def get_my_ads_keyboard(ads, user_id):
    """Генерирует список объявлений и добавляет общую кнопку массового поднятия сверху"""
    keyboard = []
...
def get_single_ad_keyboard(ad_id, last_bump, user_id):
    """Генерирует инлайн-кнопки управления одним постом"""
...
# =========================================================
# ЛОГИКА МАССОВОГО ПОДНЯТИЯ ОБЪЯВЛЕНИЙ
# =========================================================
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    for row in ad_rows:
        ...
        if remaining_time:
            ...
            # Можно поднимать! Перепубликуем пост
    ...
    # Формируем итоговое понятное сообщение для всплывающего окна
    alert_text = ""
    if bumped_count > 0:
    ...
    # Обновляем клавиатуру списка под актуальное состояние таймеров
    async with db_lock:
