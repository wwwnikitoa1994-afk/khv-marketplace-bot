...
def get_my_ads_keyboard(ads, user_id):
    """Генерирует меню со списком. КД считается до БЛИЖАЙШЕГО объявления, если хотя бы одно доступно"""
    ...
    if not is_admin(user_id) and ads:
        ...
        for ad in ads:
            rem = get_cooldown_remaining(ad[2]) # ad[2] — это last_bump
            if rem:
                try:
                ...
                # Найдено хотя бы одно объявление, у которого КД вышло
                has_available = True

        # Если есть доступные объявления, кнопка активна, КД пишем до БЛИЖАЙШЕГО (минимального) из оставшихся
        if has_available:
            if cooldown_durations:
                min_remaining = min(cooldown_durations)
                hours, remainder = divmod(int(min_remaining.total_seconds()), 3600)
                minutes, _ = divmod(remainder, 60)
                bump_all_text = f"🔄 Поднять все дост. (след. через {hours}ч {minutes}м)"
            else:
                bump_all_text = "🔄 Поднять все доступные"
        else:
            # Если вообще ни одного доступного объявления нет, КД пишем до ближайшего релиза
            if cooldown_durations:
                min_remaining = min(cooldown_durations)
                hours, remainder = divmod(int(min_remaining.total_seconds()), 3600)
                minutes, _ = divmod(remainder, 60)
...
async def edit_price(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_admin(callback.from_user.id):
        ...
        if remaining_time:
            # Оставил алерт по твоему запросу
...
async def bump_all_ads(callback: CallbackQuery):
    ...
    # Сначала проверяем, есть ли вообще хоть что-то доступное для поднятия
    for row in ads_rows:
        ad = dict(zip(DB_COLUMNS, row))
        if is_admin(user_id) or not get_cooldown_remaining(ad["last_bump"]):
            has_any_available = True
            break

    # Если ни одно объявление не готово к апдейту, выводим общий алерт
    if not has_any_available and not is_admin(user_id):
        cooldowns = []
        for row in ads_rows:
            ad = dict(zip(DB_COLUMNS, row))
            try:
                ...
                rem_td = (last_bump_dt + timedelta(hours=COOLDOWN_HOURS)) - datetime.now()
                cooldowns.append(rem_td)
            except Exception:
                pass
        min_remaining = min(cooldowns) if cooldowns else timedelta(0)
        hours, remainder = divmod(int(min_remaining.total_seconds()), 3600)
        minutes, _ = divmod(remainder, 60)
        
        await callback.answer(f"⏳ Ни одно объявление не поднято.\nБлижайшее будет доступно через: {hours}ч {minutes}м", show_alert=True)
        return

    # Если доступные объявления есть, запускаем процесс перепубликации
    for row in ads_rows:
        ad = dict(zip(DB_COLUMNS, row))
        ad_id = ad["id"]
        ...
        if not is_admin(user_id):
            remaining_time = get_cooldown_remaining(ad["last_bump"])
            if remaining_time:
                skipped_details.append(f"• ID {ad_id}: еще {remaining_time}")
                continue
    ...
    # Пересобираем меню с новым расчетом КД до ближайшего объявления
    async with db_lock:
