...
COOLDOWN_HOURS = 24  # Лимит на поднятие/изменение — 24 часа
...
def build_caption(ad):
    ...
    # 1. Первая строчка (Смайлы стоят ровно вне HTML-тегов)
    ...
    # 3. Блок описания и характеристик (Плотный список)
    ...
    if ad.get("price") and str(ad['price']).strip():
        # Скидочная цена активна только при продаже
...
async def get_description(message: Message, state: FSMContext):
    ...
    # Определение полей под каждый тип поста
    if action in ["🟢 Продам", "🟠 Обменяю"]:
        ...
        # Для объявления «Отдам» сразу пропускаем ввод характеристик и переходим к фото
...
async def get_condition(message: Message, state: FSMContext):
    ...
    if action == "🟢 Продам":
        ...
        # Для «Обменяю» запрашиваем "Интересует" вместо цены
